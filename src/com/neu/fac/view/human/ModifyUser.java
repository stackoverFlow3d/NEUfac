...
import com.neu.fac.TableMoudle.AppModel;
...
public class ModifyUser {
    ...
    @FXML
    void conform(ActionEvent event) throws IOException {
        ...
        alert.setTitle("修改用户信息");
        alert.setHeaderText("温馨提示:");
        //信息校验判断
        String message = DataCheckUtils.validateData(user);
        if (message == null) {
            flag = userController.modifyUser(DataTrans.getUserDate().getId(),user);
            if (flag) {
                alert.setContentText("用户修改成功!");
                alert.showAndWait();
    ...
    public void initialize() {
        ...
        id_text.setText(user.getUserName());
        password_text.setText(user.getPassWord());
        description_text.setText(user.getDiscription());
        facNam_text.setText(user.getFacName());
